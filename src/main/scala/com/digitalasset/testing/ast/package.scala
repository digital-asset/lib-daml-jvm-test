...
import com.digitalasset.testing.Patterns.SomeCtor
...
package object ast {
  def toAst(v: Value): Ast = v match {
    ...
    case o: DamlOptional if o.getValue.isPresent => Ast.Map(Map(SomeCtor -> toAst(o.getValue.get))) //map(v => toAstJ(v)).orElseGet(() => Ast.Null)))

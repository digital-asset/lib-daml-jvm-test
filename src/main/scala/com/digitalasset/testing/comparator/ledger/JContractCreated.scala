      ...
      override def test(event: TreeEvent): MessageTester.ComparisonResult = {
        event match {
          case event1: CreatedEvent
            ...
            val jCreateArguments = event1.getArguments
            val createArguments = com.digitalasset.ledger.api.v1.value.Record
              .parseFrom(jCreateArguments.toProtoRecord.toByteArray)
